...
Notifications.createNotification = function(userIds, params) {
  ...
  check(params, Object);
  if (!Notifications._courier[params.event])
  ...
  users.forEach(function (user) {
    ...
    _.each(_.keys(Notifications._courier[params.event].media), function (medium) {
      //check if this notification should be sent to medium
      if (sendToMedium(user, params.event, medium)) {
...
sendToMedium = function (user, event, medium) {
  var mediumObject = Notifications._courier[event].media[medium]
