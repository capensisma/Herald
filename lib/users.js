...
Herald.userPrefrence = function (user, medium, courier) { return Herald.getUserPrefrence(user, medium, courier) }
...
Herald.setUserPreference = function (user, preference, courier) {
  ...
  if (!_.isEmpty(badKeys)) throw new Error('Herald - getUserPrefrence: "'+ _.toArray(badKeys) + '" are not valid media')
  ...
  if (!user || !user._id) throw new Error('Herald - setUserPrefrence: user not found')
...
Herald.getUserPrefrence = function (user, medium, courier) {
  ...
  if (!user || !user._id) throw new Error('Herald - getUserPrefrence: user not found')

  var defaultOutput = Herald.settings.userPrefrenceDefault
