...
Notifications = {
  ...
  addCourier: function (key, options) {
    check(key, String);
    if (Notifications._courier[key]) 
      throw new Error('Notifications: courier type "' + key + '"" already exists');
    ...
    var message = function (template) {
      var message, messageFormat = Notifications._courier[key].messageFormat
      ...
      if (_.isObject(messageFormat) && !_.isFunction(messageFormat) && !_.isString(messageFormat)) {
        if (messageFormat[template]) {
          ...
          if (!message) {
            throw new Meteor.Error('Notifications: No default message defined for "' + this.courier + '" notifications');
          }
        }
      } 
      ...
      if (_.isFunction(message)) {
        ...
        throw new Meteor.Error('Notifications: message not defined for "' + this.courier + '" notifications');
      }
    }


    check(options, Object);
    Notifications._courier[key] = {
      message: message,
    ...
    if (Meteor.isServer) {
      ...
      if (media.length == 0)
        throw new Error('Notifications: courier "'+ key + '" must have at least one medium');
      media.forEach(function (medium) {
        if (!_.contains(Notifications._media, medium)) 
          throw new Error('Notifications: medium "' + medium + '" is not a known media');
      });
    }
    //define on both, just in case
    Notifications._courier[key].media = options.media

    //white-listed params from extension packages
    _.extend(Notifications._courier[key], _.pick(options, Notifications._extentionParams)) 
  }
};
  ...
  transform: function (notification) {
    if (notification.courier) { //courier may not be available if fields filter was called.
      ...
      notification.message = function (template) {
        ...
        if (Notifications._courier[this.courier].message) {
          //make the notification data accessible to the message function.
          return Notifications._courier[this.courier].message.call(this, template)
        };
      };

      //Load the current metadata, this will update with a hot-code-push.
      notification.metadata = Notifications._courier[notification.courier].metadata
    };
    return notification
  }
});
