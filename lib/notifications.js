...
Notifications = {
  ...
  addEventType: function (key, options) {
    check(key, String);
    if (Notifications._eventTypes[key]) 
    ...
    var message = function (template) {
      var message, messageFormat = Notifications._eventTypes[key].messageFormat
    ...
    if (Meteor.isServer) {
      ...
      options.media.forEach(function (medium) {
        if (!_.contains(Notifications._media, medium.name)) 
          throw new Error('Notifications: medium "' + medium.name + '" is not a known media');
      });
    }
    //define on both, just in case
    Notifications._eventTypes[key].media = options.media

    //white-listed params from extension packages
    _.extend(Notifications._eventTypes[key], _.pick(options, Notifications._extentionParams)) 
  }
};
  ...
  transform: function (notification) {
    if (notification.event) { //event may not be available if fields filter was called.
      ...
      notification.message = function (template) {
        ...
        if (Notifications._eventTypes[this.event].message) {
          //make the notification data accessible to the message function.
          return Notifications._eventTypes[this.event].message.call(this, template)
        };
      };

      //Load the current metadata, this will update with a hot-code-push.
      notification.metadata = Notifications._eventTypes[notification.event].metadata
    };
    return notification
  }
});
