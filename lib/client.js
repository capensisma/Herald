...
Meteor.startup(function () {
  ...
  var runnersQuery = [], runners = [];
  _.each(_.keys(Herald._mediaRunnersClient), function (runner) {
    runners.push(runner)
    var query = {};
    query[runner] = true;
  ...
  Herald.collection.find({$or: runnersQuery, read: false}).observe({
    added: function (notification) {
      ...
      Herald.collection.update(notification._id, {$set: { runners: _.omit(notification.runners, runners) } });
    }
  });
});
